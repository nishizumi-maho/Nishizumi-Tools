...
Usage
-----
    python Nishizumi_CautionOverlay.py            # normal run, connects to iRacing
    python Nishizumi_CautionOverlay.py --demo     # fake data, for checking the layout

The overlay also ships inside the Nishizumi Tools launcher, which opens it in
its own process and points it at the shared data folder (``NISHIZUMI_DATA_DIR``)
so the window position, size and lock state live next to the other apps.
...
# Written inside the Nishizumi Tools data folder when the launcher starts the
# overlay, so every app in the suite keeps its state in the same place.
SETTINGS_FILE = "nishizumi_caution_overlay.ini"
DATA_DIR_ENV = "NISHIZUMI_DATA_DIR"
...
def open_settings() -> QSettings:
    """Where the window position, size and lock state are stored.

    Standalone the overlay uses the platform default (the registry on Windows).
    Started from the Nishizumi Tools launcher it writes an INI file inside the
    shared data folder instead, so the whole suite keeps its state together.
    """
    ...
    if data_dir:
        try:
            ...
            return QSettings(str(folder / SETTINGS_FILE), QSettings.IniFormat)
        except Exception:
            pass
    return QSettings(ORG_NAME, SETTINGS_APP)
...
def _build_app(argv: list[str] | None = None) -> QApplication:
...
def run_overlay(demo: bool = False, argv: list[str] | None = None) -> int:
    """Show the overlay and run it until the window is closed.

    This is the entry point the Nishizumi Tools launcher imports; it starts the
    overlay without going through the command line parser, because the launcher
    passes its own arguments to the process.
    """
    app = _build_app(argv if argv is not None else sys.argv[:1])
    window = OverlayWindow(demo=demo)
    window.show()
    return app.exec()
...
def main() -> int:
    ...
    return run_overlay(demo=args.demo)
